...
"sqlDrop"<-
function(channel,sqtable,errors=T)
{
	if (channel < 0)			
...
"sqlFetch" <-
function (channel, sqtable, errors = T, as.is = F, colnames = F, 
    rownames = F) 
{
    if (channel < 0) 
        stop("invalid channel")
	...
	toupper=tablename <- odbctoupper(tablename),
	tolower=tablename <- odbctolower(tablename)
	)
    if (!any(sqlTables(channel)[3] == tablename)) 
        stop(paste(tablename, ":table not found on channel", 
            channel))
    query <- paste("SELECT * FROM", tablename)
    ans <- sqlQuery(channel, query, errors = errors, as.is = as.is)
...
"sqlCopy" <- 
function(channel,query,destination,destchannel=-1,verbose=T,errors=T)
{
	...
	if(length(tablename) != 1)
		stop(paste(tablename,"should be a name"))
	dataset<-sqlQuery(channel,query)
	assign(as.character(substitute(destination)),dataset)
	sqlSave(destchannel,destination,verbose=verbose)
	}

"sqlCopyTable"<-
function (channel,srctable,desttable,destchannel=-1,verbose=T,errors=T)
{	
	if (channel < 0)	
		stop("invalid channel")
	...
	query<-sqltablecreate(dtablename,sqlColumns(channel,stablename),sqlPrimaryKeys(channel,stablename))
	if(verbose)
		print(query)
...
"sqlSave" <- 
function(channel,dat,tablename=NULL,append=F,rownames=F,colnames=F,verbose=F,test=F,fast=T)
{
	if (channel < 0)			
		stop("invalid channel")
	if(missing(dat))
	...
	if(length(tablename) != 1)
	    ...
	    switch(odbcCaseFlag(channel),
		toupper=colnames(dat) <- odbctoupper(colnames(dat)),
		tolower=colnames(dat) <- odbctolower(colnames(dat))
		)
	...
	if(any(sqlTables(channel)[3]==tablename)){
		...
		if(sqlwrite(channel,tablename,dat,test=test,fast=fast)== -1){
...
#	No permissions for existing table should have crashed above
		...
		newname<-as.data.frame(cbind(V1=NA,V2=NA,V3=tablename,V4=names(dat),V5=12,V6='varchar',V7=255,V8=255,V9=NA,V10=NA,V11=1,V12=NA))
		query<-sqltablecreate(tablename,newname,keys=0)
		if(verbose)
			print(query)
		#last chance:  let it die if fails
		sqlQuery(channel,query,errors=T)
		if(sqlwrite(channel,tablename,dat,test=test,fast=fast) < 0){
...
"sqlwrite" <-
function (channel, tablename, mydata, test=F,fast = T) 
{
    ...
    for (i in c(1:nrow(mydata))) {
        ...
        for (j in c(1:ncol(mydata))) {
            nextdata <- as.character(mydata[i, j])
            if (is.na(nextdata)) {
                nextdata <- "NULL"
            }
            if (!is.na(grep("char", odbctolower(as.character(coldata[j, 
                1])))[1])) {
                nextdata <- paste("'", nextdata, sep = "")
                nextdata <- paste(nextdata, "'", sep = "")
            }
            if (fast) 
                query <- paste(query, "?")
            else query <- paste(query, nextdata)
            if (j < ncol(mydata)) {
                query <- paste(query, ",")
            }
            else {
...
if(fast){
	coldata <- sqlColumns(channel, tablename)[c(4, 5, 
		7, 8, 9)]
	...
	paramdata <- as.character(as.matrix(coldata[coldata[, 
		1] == colnames[1], ]))
	for (i in 2:length(colnames)) {
		paramdata <- cbind(paramdata, as.character(as.matrix(coldata[coldata[, 
		  1] == colnames[i], ])))
		}
	stat <- odbcUpdate(channel, query, mydata, paramdata, 
...
function (tablename, coldata,keys=-1) 
{
	...
	if(coldata[i,7]==65535){ # no size parameter
        ...
        create <- paste(create, gsub("[^A-Za-z0-9]+","",as.character(coldata[i,4]))," ", coldata[i,6],
	colsize, null,sep="")
        if(!is.numeric(keys)){
...
###############################################
"sqlTables" <- 
function(channel,errors=F,as.is=F){
	...
	if (stat < 0)			
	{
		...
		return(sqlGetResults(channel,as.is=as.is,transposing=F))
	}
###

"sqlColumns" <-
function (channel, sqtable, errors = F, as.is = F,special=F) 
{
	...
	if(any(odbctolower(sqlTables(channel)[,3]) == odbctolower(sqtable))){
    ...
    else return(sqlGetResults(channel, as.is = as.is, transposing = F))
}

"sqlPrimaryKeys" <- 
function(channel,sqtable,as.is=F,errors=F){
	if(!is.character(sqtable))
		sqtable<-as.character(substitute(sqtable))
	if(length(sqtable) != 1)
		stop(paste(sqtable,"should be a name"))
...
"sqlQuery" <- 
function(channel,query,errors=T,as.is=F,transposing=F,max=0,buffsize=1000)
{
	if(missing(query))
		stop("Missing parameter")
	stat <- odbcQuery(channel, query)
	if (stat < 0)			
	{
		...
		return(sqlGetResults(channel,as.is=as.is,errors=errors,transposing=transposing,max=max,buffsize=buffsize)) 		
}

#	See below.  Retained for safekeeping
#
"sqlGetResults.old" <-
function(channel,as.is=F,errors=F,transposing=F,dec=".")
{
...
"sqlGetResults" <-
function (channel, as.is = F, errors = F, transposing = F, max = 0, 
    buffsize = 1000)
{
        type.convert <- function(x, na.strings = "NA", as.is = FALSE) .Internal(type.convert(x, 
            na.strings, as.is, dec = options("dec")$dec))
    cols <- odbcNumCols(channel)$num
    if (cols < 0) {
        if (errors){ 
            return("Nooo data")
    ...
    attrnames <- c(odbcColData(channel))$names
    dbdata <- odbcFetchRows(channel, transposing = transposing, 
        max = max, buffsize = buffsize)
    if (dbdata$stat == -1) {
	if(errors)
		return(odbcGetErrMsg(channel))
	else
    ...
    if (errors) 
        if (any(odbcColData(channel)$length > 255)) 
            print("Warning: a column has been truncated")
    dbdata <- as.data.frame(dbdata$data)
    cols <- ncol(dbdata)
    if (is.logical(as.is)) {
        as.is <- rep(as.is, length = cols)
    }
    ...
    for (i in 1:cols) if (!as.is[i]) 
        dbdata[[i]] <- type.convert(as.character(dbdata[[i]]))
    if (transposing) 
        rownames(dbdata) <- attrnames
    else colnames(dbdata) <- attrnames
    return(dbdata)
}
#################################################
"sqlUpdate" <-
function(channel,dat,verbose=F,test=F)
{
	if (channel < 0)			
		stop("invalid channel")
	if(missing(dat))
	...
	# this may or may not include the unique column dependig
	...
	# its descriptor data
	indexcols<-(sqlColumns(channel,tablename,special=T))
	if(any(indexcols==-1))
	...
	if(!any(indexflags))
		stop(paste("index column",paste(as.character(indexcols),collapse=" "), "not in dataframe"))
	...
	# this nex bit of twiddling extracts the descriptor data for each
	...
	for (i in 2:length(paramnames)){
		paramdata<-cbind(paramdata,as.character(as.matrix(coldata[coldata[,1]==paramnames[i],])))
	}
