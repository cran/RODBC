...
a wide range of platforms (and \code{iODBC} shipped with versions of
OS X prior to 10.9).  The connection to the particular DBMS needs
an \emph{ODBC driver}: these may come with the DBMS or the ODBC driver
manager or be provided separately by the DBMS developers, and there
are third-party\footnote{but there are close links between
...
spreadsheets. Actual Technologies sell a driver for OS X that covers
...
\item[Character types] Character types can be classified three ways:
  ...
  character data, which could be UTF-8 (and will be on OS X and in
...
OS X Excel driver: however, it allows only read-only access to
...
Actual Technologies sell an SQLite driver for OS X which requires
...
available for Windows and OS X (before 10.9) from CRAN.
...
be installed separately if required).  OS X from 10.2 to 10.8 shipped
...
included in the OS X SDK.  The CRAN OS X binary package for 10.8 was
...
sources under OS X: they need ODBC drivers compiled for the driver
manager in use.  At least for OS X 10.9--11, all that is required for
...
An SQLite ODBC driver for Unix-alikes, including OS X, and (32-
...
For recent versions of OS X, low-cost and easy-to-use \code{iODBC}
...
OS X drivers for MySQL, PostgreSQL and the major commercial databases
...
\end{example}
For \code{iODBC} use \code{iodbc\_config}: on OS X the system
...
OS X prior to 10.6 came with a very similar GUI (figure~3) found at
\textsf{Applications / Utilities / ODBC Administrator}.  This has been
available as a download from \url{http://support.apple.com/kb/DL895}.
Another GUI for OS X is available from
