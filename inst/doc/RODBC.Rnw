...
\code{Date} and \code{POSIXt}), ODBC has both C and SQL data types,
...
\item[Character types] Character types can be classified three ways:
  ...
  to transfer character data, and so such types may be subjected to a
  fixed limit or not work at all.
...
The Microsoft Excel ODBC driver (Windows only) has a number of
...
It seems that its concept of a `table' is principally a \emph{named
  range}. It treats worksheets are system tables, and append a dollar
...
SQLite~ODBC driver was easy to install from the sources and works
correctly.)
...
MyODBC') in source form, and binaries for all common R platforms.

PostgreSQL has an associated project at
\url{http://pgfoundry.org/projects/psqlodbc/} and another project for
a driver in development at
\url{http://pgfoundry.org/projects/odbcng/}.  (Documentation for
...
for some of its platforms (including 32-bit Windows and \verb|ix86|
Linux, but not Mac OS X nor \verb|x86_64| Linux).  See
\url{http://www.oracle.com/technology/software/tech/oci/instantclient/}.
One quirk of the Windows driver is that the Oracle binaries must be in
the path, so \code{PATH} should include e.g{} \verb|c:\Oracle\bin|.
...
Some details of the Microsoft `ODBC Desktop Database Drivers' (for
Access, Excel, Paradox, dBase and text files on Windows) can be found
at
...
Windows drivers for Access 2007 and Excel 2007 are bundled with Office
2007 but can be installed separately \emph{via} the installer
...
Windows has a GUI to set up DSNs, called something like `Data Sources
(ODBC)' under `Administrative Tools' in the Control Panel. You can
add, remove and edit (`configure') DSNs there (see figure~2).  When
adding a DSN, first select the ODBC driver and then complete the
...
\code{iODBC} also has a web-based DSN administrator.  UnixODBC's GUI
...
(ncluding for the end of the R session), with a warning unless the
connection has already been closed by \code{close} or \code{odbcClose}.
In addition, a C-level table keeps the pointers of the first 1000
connections of an R session, to enable \code{odbcCloseAll} to close them.
...
\code{sqlSave} and \code{sqlUpdate}.  These use \code{INSERT INTO} and
\code{UPDATE} queries respectively, and for \code{fast = TRUE} use
